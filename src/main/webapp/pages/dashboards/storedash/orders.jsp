...
<%
    ...
    // Fetch paid orders from database for this store only
...
<head>
  ...
  <style>
    /* Orders-specific styles (not in framework.css) */
    ...
    .status.cancelled { background-color: var(--destructive); }
    ...
    /* Refund button */
    .btn-refund, .refund-btn {
      background-color: #dc3545;
      color: white;
    }

    /* View/Update/Delivery buttons */
    .btn-view, .view-btn { background-color: var(--primary); color: var(--primary-foreground); }
    .btn-update, .update-btn { background-color: var(--primary); color: var(--primary-foreground); }
    .btn-delivery, .delivery-btn { background-color: var(--destructive); color: var(--destructive-foreground); }
    ...
    .vehicle-modal h3 { color: var(--primary); margin-bottom: 20px; }
    ...
    .vehicle-modal .confirm-btn { background: var(--primary); color: var(--primary-foreground); }
    .vehicle-modal .confirm-btn:hover { opacity: 0.8; }
    .vehicle-modal .cancel-btn { background: var(--secondary); color: var(--secondary-foreground); border: 1px solid var(--border); }
    .vehicle-modal .cancel-btn:hover { background: var(--accent); color: var(--accent-foreground); }
    ...
    .order-details-content .detail-section { margin-bottom: 20px; }
    ...
    .order-details-content .detail-row:last-child { border-bottom: none; }
    .order-details-content .detail-label { font-weight: 600; color: var(--muted-foreground); }
    .order-details-content .detail-value { color: var(--foreground); }
    ...
    .order-details-content .close-order-modal:hover { color: var(--primary); }
    ...
    .refund-modal .refund-modal-content {
      background: var(--card);
      color: var(--card-foreground);
      border-radius: var(--radius-lg);
      padding: 30px;
    ...
    .refund-modal h3 { color: var(--destructive); margin-bottom: 15px; }
    .refund-modal .reason-options {
      display: flex;
      flex-direction: column;
      gap: 8px;
      margin-bottom: 15px;
    }
    .refund-modal .reason-btn {
      padding: 10px 15px;
      border: 2px solid var(--border);
      border-radius: var(--radius-md);
      background: var(--card);
    ...
    .refund-modal .reason-btn:hover { border-color: var(--primary); background: var(--accent); }
    .refund-modal .reason-btn.selected { border-color: var(--destructive); background: var(--muted); }
    .refund-modal .custom-reason {
      width: 100%;
      padding: 10px;
      border: 1px solid var(--border);
      border-radius: var(--radius-md);
    ...
    .refund-modal .modal-buttons {
      ...
      margin-top: 20px;
                      ...
                      <body class="dashboard-layout">
                        ...
                        <aside class="sidebar">
                          ...
                          <ul>
                            ...
                            <li><a href="${pageContext.request.contextPath}/StoreReviewsServlet">Customer Reviews</a>
                            </li>
                            <li><a
                ...
                <% } else {
                    ...
                    for (Order order : orders) {
                        ...
                        if ("PENDING".equals(dbStatus)) {
                            ...
                            displayStatus = "Processing"; statusClass = "processing";
                        } else if ("OUT_FOR_DELIVERY".equals(dbStatus) || "OUT FOR DELIVERY".equals(dbStatus)) {
                            displayStatus = "Out for Delivery"; statusClass = "out-delivery";
                        } else if ("DELIVERED".equals(dbStatus)) {
                            displayStatus = "Delivered"; statusClass = "delivered";
                        } else if ("PAID".equals(dbStatus)) {
                            displayStatus = "Pending"; statusClass = "pending";
                        } else if ("REFUNDED".equals(dbStatus)) {
                            displayStatus = "Refunded"; statusClass = "refunded";
                        } else if ("CANCELLED".equals(dbStatus)) {
                            displayStatus = "Cancelled"; statusClass = "cancelled";
                        }
                ...
                <tr>
                    ...
                    <td><%= totalAmount %></td>
                    <td>
                        <button class="btn view-btn" data-order-id="<%= orderId %>" data-customer-name="<%= escapedCustomerName %>" data-total="<%= totalAmount %>" data-address="<%= escapedAddress %>" data-city="<%= escapedCity %>" data-phone="<%= escapedPhone %>" data-email="<%= escapedEmail %>" data-order-date="<%= orderDate %>" data-order-items='<%= orderItemsJsonStr %>' onclick="showOrderDetailsModalFromButton(this)">View Details</button>
                        ...
                        <% if ("PAID".equals(dbStatus) || "PROCESSING".equals(dbStatus)) { %>
                            <button class="btn refund-btn" onclick="showRefundModal('<%= orderId %>')">Cancel &amp; Refund</button>
                        <% } %>
                    </td>
                </tr>
                <% orderIndex++; } } %>
                          ...
                          <div class="order-details-content">
                            ...
                            <div class="detail-section">
                              ...
                              <div class="detail-row">
                                ...
                                <span class="detail-value" id="modal-total"
                                  style="font-weight: 600; color: oklch(0.5393 0.2713 286.7462); font-size: 1.1em;">-</span>
                              </div>
                            </div>
                          </div>
                        </div>
                        ...
                        <div id="refundModal" class="refund-modal">
                          <div class="refund-modal-content">
                            ...
                            <p style="color: var(--muted-foreground); margin-bottom: 15px;">Order: <strong
                                id="refundOrderIdDisplay"></strong></p>
                            <p style="font-weight: 600; margin-bottom: 8px;">Select a reason for cancellation:</p>
                            <div class="reason-options">
                              <button class="reason-btn" onclick="selectRefundReason(this, 'Out of stock')">Out of
                                stock</button>
                              <button class="reason-btn"
                                onclick="selectRefundReason(this, 'Product unavailable')">Product unavailable</button>
                              <button class="reason-btn" onclick="selectRefundReason(this, 'Pricing error')">Pricing
                                error</button>
                              <button class="reason-btn" onclick="selectRefundReason(this, 'Other')">Other (specify
                                below)</button>
                            </div>
                            <textarea class="custom-reason" id="customRefundReason"
                              placeholder="Enter custom reason..."></textarea>
                            <div class="modal-buttons">
                              <button class="modal-btn cancel-btn" onclick="closeRefundModal()">Go Back</button>
                              <button class="modal-btn confirm-btn" id="confirmRefundBtn" onclick="confirmRefund()"
                                style="background: var(--destructive); color: white;">Confirm Refund</button>
                            </div>
                          </div>
                        </div>
                        ...
                        <script>
                          ...
                          function changeStatus(button, newStatus, orderId, orderIndex) {
                            ...
                            const statusMap = {
                              ...
                              'REFUNDED': { text: 'Refunded', cssClass: 'refunded' },
                              'CANCELLED': { text: 'Cancelled', cssClass: 'cancelled' }
                            };
                        ...
                        <script>
                          ...
                          function showRefundModal(orderId) {
                            ...
                            refundReason = '';
                            document.getElementById('refundOrderIdDisplay').textContent = '#' + orderId;
                            document.getElementById('customRefundReason').value = '';
                            document.getElementById('customRefundReason').style.display = 'none';
                          ...
                          function selectRefundReason(btn, reason) {
                            ...
                            const customInput = document.getElementById('customRefundReason');
                            if (reason === 'Other') {
                          ...
                          function confirmRefund() {
                            ...
                            if (finalReason === 'Other') {
                              finalReason = document.getElementById('customRefundReason').value.trim();
                              if (!finalReason) {
                            ...
                            fetch(contextPath + '/refund', {
                              ...
                              body: 'orderId=' + encodeURIComponent(refundOrderId) + '&cancelledBy=store&reason=' + encodeURIComponent(finalReason)
                            })
                              .then(response => response.json())
                              .then(data => {
                                closeRefundModal();
                                if (data.success) {
                                  alert('Order cancelled and refund initiated. ' + (data.message || ''));
                                  location.reload();
                                } else {
                          ...
                          // Close refund modal on background click
